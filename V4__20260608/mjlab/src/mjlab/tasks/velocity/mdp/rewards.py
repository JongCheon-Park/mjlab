...
from mjlab.envs.mdp.rewards import action_rate_l2 as env_action_rate_l2
...
def _score_with_target_error_penalty(
  norm_err_sq: torch.Tensor,
  *,
  raw_err_sq: torch.Tensor,
  penalty_weight: float,
) -> torch.Tensor:
  score = torch.exp(-norm_err_sq)
  if penalty_weight == 0.0:
    return score
  penalized_score = score - penalty_weight * raw_err_sq
  return torch.where(penalized_score <= 0.0, penalized_score, score)


def _relaxed_log_barrier(z: torch.Tensor, delta: float | torch.Tensor) -> torch.Tensor:
  """Relaxed logarithmic barrier for a one-sided constraint ``z >= 0``.

  This follows the barrier-style reward used by Kim et al. (ICRA 2025):
  ``B(z;δ)=log(z)`` for ``z > δ`` and a quadratic relaxation for ``z <= δ``.
  ``delta`` is the distance from the constraint boundary where the reward
  switches from the interior log branch to the relaxed quadratic branch.
  """
  if isinstance(delta, torch.Tensor):
    delta_t = delta.to(dtype=z.dtype, device=z.device)
  else:
    if delta <= 0.0:
      raise ValueError(f"Relaxed log barrier delta must be positive, got {delta!r}.")
    delta_t = torch.as_tensor(delta, dtype=z.dtype, device=z.device)
  log_branch = torch.log(torch.where(z > delta_t, z, delta_t))
  quadratic_branch = torch.log(delta_t) - 0.5 * (
    torch.square((z - 2.0 * delta_t) / delta_t) - 1.0
  )
  return torch.where(z > delta_t, log_branch, quadratic_branch)
...
class biped_air_time:
  ...
  def __call__(
    ...
    if std_sidewalk is not None:
      selected_std = torch.where(
        lateral_only_mask,
        torch.full_like(selected_std, float(std_sidewalk)),
        selected_std,
      )
...
class biped_first_swing_foot:
  ...
  @staticmethod
  def _left_right_foot_indices(sensor: ContactSensor) -> tuple[int, int]:
    """Return sensor data indices for left and right feet, falling back to left-right order."""
    slots = getattr(sensor, "__dict__", {}).get("_slots", ())
    primary_names: list[str] = []
    for slot in slots:
      primary_name = getattr(slot, "primary_name", None)
      if isinstance(primary_name, str) and primary_name not in primary_names:
    ...
    for index, name in enumerate(primary_names):
      ...
      if left_index is None and "left" in name_lower:
        left_index = index
      if right_index is None and "right" in name_lower:
        right_index = index
  ...
  def _load_transfer_force_diff_norm(
    ...
    valid = (desired_foot >= 0) & (desired_foot < num_feet)
    swing_foot = torch.clamp(desired_foot, min=0, max=num_feet - 1)
    left_index, right_index = self._left_right_foot_indices(sensor)
    left_foot = torch.full_like(desired_foot, left_index)
    right_foot = torch.full_like(desired_foot, right_index)
    support_foot = torch.where(swing_foot == left_foot, right_foot, left_foot)
    support_foot = torch.clamp(support_foot, min=0, max=num_feet - 1)
  ...
  def __call__(
    ...
    if torch.any(arm_now):
      ...
      left_index, right_index = self._left_right_foot_indices(sensor)
      left_foot = torch.full_like(self._desired_foot, left_index)
      right_foot = torch.full_like(self._desired_foot, right_index)
...
def biped_standing_stability(
  ...
  standing_action_rate_normalize: bool = False,
) -> torch.Tensor:
  ...
  action_rate_cost = env_action_rate_l2(env, normalize=standing_action_rate_normalize)
...
class knee_height:
  """Reward knees for rising upward relative to pelvis/base.

  This mirrors the simple command-gated structure of ``feet_clearance`` but uses
  configured knee bodies instead of foot sites. Heights, forward positions, and
  vertical velocities are measured relative to a configured reference body, such
  as pelvis/base. The forward axis uses the reference body's yaw frame.

  Each configured knee contributes:

  ``height_score = clamp((z_rel - z0_rel) / target_height_delta, 0, 1)``
  ``up_gate = 1 if vz_rel > up_velocity_threshold else 0``
  ``front_gate = 1 if x_rel > front_position_threshold else 0``
  ``target_gate = 1 until the knee reaches target_height_delta else 0``
  ``score = height_score * up_gate * front_gate * target_gate``

  The final reward is ``max(left, right)`` so one high rising knee is enough and
  lifting both knees does not increase the reward further.

  Thus lifting a knee toward ``initial + target_height_delta`` increases reward,
  but the reward turns off once the knee reaches the target height. The knee
  re-arms after returning to the initial reference height, preventing a policy
  from being rewarded for holding a leg up.
  """

  def __init__(self, cfg: RewardTermCfg, env: ManagerBasedRlEnv) -> None:
    self._asset_cfg: SceneEntityCfg = cfg.params.get("asset_cfg", _DEFAULT_ASSET_CFG)
    self._reference_asset_cfg: SceneEntityCfg | None = cfg.params.get(
      "reference_asset_cfg"
    )
    self._target_height_delta = float(cfg.params.get("target_height_delta", 0.10))
    self._up_velocity_threshold = float(cfg.params.get("up_velocity_threshold", 0.05))
    self._front_position_threshold = float(
      cfg.params.get("front_position_threshold", 0.0)
    )
    self._command_name: str | None = cfg.params.get("command_name")
    self._command_threshold = float(cfg.params.get("command_threshold", 0.01))
    self._asset_name = self._asset_cfg.name
    self._initial_height = torch.zeros(env.num_envs, 0, device=env.device)
    self._has_reference = torch.zeros(
      env.num_envs, 0, device=env.device, dtype=torch.bool
    )
    self._target_reached = torch.zeros(
      env.num_envs, 0, device=env.device, dtype=torch.bool
    )
    self._validate_target_height_delta(self._target_height_delta)
    if self._up_velocity_threshold < 0.0:
      raise ValueError(
        "knee_height up_velocity_threshold must be non-negative, "
        f"got {self._up_velocity_threshold!r}."
      )
    if cfg.weight != 0.0:
      self._cache_initial_height(env, self._asset_cfg, self._reference_asset_cfg)

  def __call__(
    self,
    env: ManagerBasedRlEnv,
    target_height_delta: float | None = None,
    command_name: str | None = None,
    command_threshold: float | None = None,
    up_velocity_threshold: float | None = None,
    front_position_threshold: float | None = None,
    asset_cfg: SceneEntityCfg | None = None,
    reference_asset_cfg: SceneEntityCfg | None = None,
  ) -> torch.Tensor:
    target_height_delta = (
      self._target_height_delta
      if target_height_delta is None
      else float(target_height_delta)
    )
    command_name = self._command_name if command_name is None else command_name
    command_threshold = (
      self._command_threshold if command_threshold is None else float(command_threshold)
    )
    up_velocity_threshold = (
      self._up_velocity_threshold
      if up_velocity_threshold is None
      else float(up_velocity_threshold)
    )
    front_position_threshold = (
      self._front_position_threshold
      if front_position_threshold is None
      else float(front_position_threshold)
    )
    asset_cfg = self._asset_cfg if asset_cfg is None else asset_cfg
    reference_asset_cfg = (
      self._reference_asset_cfg if reference_asset_cfg is None else reference_asset_cfg
    )
    self._validate_target_height_delta(target_height_delta)
    if up_velocity_threshold < 0.0:
      raise ValueError(
        "knee_height up_velocity_threshold must be non-negative, "
        f"got {up_velocity_threshold!r}."
      )
    current_forward, current_height, current_vertical_velocity = (
      self._knee_relative_state(env, asset_cfg, reference_asset_cfg)
    )
    if (
      current_forward is None
      or current_height is None
      or current_vertical_velocity is None
    ):
      return torch.zeros(env.num_envs, device=env.device)
    if self._initial_height.shape != current_height.shape:
      self._initial_height = torch.zeros_like(current_height)
      self._has_reference = torch.zeros_like(current_height, dtype=torch.bool)
      self._target_reached = torch.zeros_like(current_height, dtype=torch.bool)
    if torch.any(~self._has_reference):
      self._cache_initial_height(env, asset_cfg, reference_asset_cfg)

    height_delta = current_height - self._initial_height
    if self._target_reached.shape != current_height.shape:
      self._target_reached = torch.zeros_like(current_height, dtype=torch.bool)
    self._target_reached = torch.where(
      height_delta <= 0.0,
      torch.zeros_like(self._target_reached),
      self._target_reached,
    )
    target_tolerance = torch.finfo(height_delta.dtype).eps
    reached_now = height_delta >= target_height_delta - target_tolerance
    target_gate = (~self._target_reached & ~reached_now).float()
    height_score = (height_delta / target_height_delta).clamp(min=0.0, max=1.0)
    up_gate = (current_vertical_velocity > up_velocity_threshold).float()
    front_gate = (current_forward > front_position_threshold).float()
    reward = torch.max(height_score * up_gate * front_gate * target_gate, dim=1).values
    self._target_reached = self._target_reached | reached_now
    if command_name is not None:
      command = env.command_manager.get_command(command_name)
      if command is not None:
        linear_norm = torch.norm(command[:, :2], dim=1)
  ...
  def _cache_initial_height(
    self,
    env: ManagerBasedRlEnv,
    asset_cfg: SceneEntityCfg,
    reference_asset_cfg: SceneEntityCfg | None = None,
  ) -> None:
    _, current_height, _ = self._knee_relative_state(
      env, asset_cfg, reference_asset_cfg
    )
    ...
    if self._initial_height.shape != current_height.shape:
      ...
      self._has_reference = torch.zeros_like(current_height, dtype=torch.bool)
      self._target_reached = torch.zeros_like(current_height, dtype=torch.bool)
    missing_reference = ~self._has_reference
    self._initial_height = torch.where(
      missing_reference,
      current_height.detach(),
      self._initial_height,
    )
    self._has_reference = self._has_reference | missing_reference

  def _knee_relative_state(
    self,
    env: ManagerBasedRlEnv,
    asset_cfg: SceneEntityCfg,
    reference_asset_cfg: SceneEntityCfg | None,
  ) -> tuple[torch.Tensor | None, torch.Tensor | None, torch.Tensor | None]:
    asset: Entity = env.scene[asset_cfg.name]
    body_ids = asset_cfg.body_ids
    if isinstance(body_ids, list) and len(body_ids) == 0:
      return None, None, None
    knee_pos_w = asset.data.body_link_pos_w[:, body_ids, :]
    knee_vel_w = asset.data.body_link_lin_vel_w[:, body_ids, :]
    if knee_pos_w.shape[1] == 0:
      return None, None, None
    reference_pos_w, reference_vel_w, reference_quat_w = self._reference_state(
      env, reference_asset_cfg
    )
    knee_delta_w = knee_pos_w - reference_pos_w.unsqueeze(1)
    knee_rel_vel_w = knee_vel_w - reference_vel_w.unsqueeze(1)
    reference_yaw_quat = (
      yaw_quat(reference_quat_w).unsqueeze(1).expand(-1, knee_delta_w.shape[1], -1)
    )
    knee_delta_yaw = quat_apply_inverse(
      reference_yaw_quat.reshape(-1, 4),
      knee_delta_w.reshape(-1, 3),
    ).reshape_as(knee_delta_w)
    return knee_delta_yaw[..., 0], knee_delta_w[..., 2], knee_rel_vel_w[..., 2]
  ...
  def _reference_state(
    ...
    reference_asset: Entity = env.scene[reference_asset_cfg.name]
    body_ids = reference_asset_cfg.body_ids
    if isinstance(body_ids, list) and len(body_ids) == 0:
      return (
        reference_asset.data.root_link_pos_w,
        reference_asset.data.root_link_lin_vel_w,
        reference_asset.data.root_link_quat_w,
      )
    reference_pos_w = reference_asset.data.body_link_pos_w[:, body_ids, :]
    if reference_pos_w.shape[1] == 0:
      return (
        reference_asset.data.root_link_pos_w,
        reference_asset.data.root_link_lin_vel_w,
        reference_asset.data.root_link_quat_w,
      )
    reference_vel_w = reference_asset.data.body_link_lin_vel_w[:, body_ids, :]
    reference_quat_w = reference_asset.data.body_link_quat_w[:, body_ids, :]
    return (
      reference_pos_w.mean(dim=1),
      reference_vel_w.mean(dim=1),
      reference_quat_w[:, 0, :],
    )

  @staticmethod
  def _validate_target_height_delta(target_height_delta: float) -> None:
    if target_height_delta <= 0.0:
      raise ValueError(
        "knee_height target_height_delta must be positive, "
        f"got {target_height_delta!r}."
      )

  def reset(self, env_ids: torch.Tensor | slice | None = None) -> None:
    if env_ids is None:
      env_ids = slice(None)
    self._has_reference[env_ids] = False
...
def _contact_found_by_side(sensor: ContactSensor, found: torch.Tensor) -> torch.Tensor:
  """Collapse contact columns into ``[right, left]`` boolean foot contacts."""
  if found.ndim == 1:
    found = found.unsqueeze(-1)

  contacts: list[torch.Tensor] = []
  primary_names = getattr(sensor, "primary_names", ())
  for side in ("right", "left"):
    cols = [
      i
      for i, name in enumerate(primary_names)
      if isinstance(name, str) and name.startswith(f"{side}_")
    ]
    if not cols:
      cfg = getattr(sensor, "cfg", None)
      raise ValueError(
        f"Contact sensor '{getattr(cfg, 'name', '<unknown>')}' does not "
        f"contain a {side!r} primary. Available primaries: {tuple(primary_names)}"
      )
    cols_tensor = torch.tensor(cols, device=found.device, dtype=torch.long)
    contacts.append(torch.any(found[:, cols_tensor] > 0, dim=1))
  return torch.stack(contacts, dim=1)


def foot_center_noncontact(
  env: ManagerBasedRlEnv,
  foot_sensor_name: str,
  center_sensor_name: str,
) -> torch.Tensor:
  """Penalize feet that contact terrain without touching their center sole geom.

  This replaces orientation-based ``foot_flat`` shaping with an explicit contact
  condition: if a whole-foot terrain contact exists, at least one center sole
  collision geom on that same foot must also be in contact. The term is binary
  per foot and does not scale with contact force, avoiding force-inflation
  incentives.
  """
  foot_sensor: ContactSensor = env.scene[foot_sensor_name]
  center_sensor: ContactSensor = env.scene[center_sensor_name]

  foot_found = foot_sensor.data.found
  center_found = center_sensor.data.found
  assert foot_found is not None, (
    f"Contact sensor '{foot_sensor_name}' must include the 'found' field."
  )
  assert center_found is not None, (
    f"Contact sensor '{center_sensor_name}' must include the 'found' field."
  )

  foot_contact = _contact_found_by_side(foot_sensor, foot_found)
  center_contact = _contact_found_by_side(center_sensor, center_found)
  missing_center = foot_contact & ~center_contact
  return torch.sum(missing_center.float(), dim=1)
...
def _normalized_foot_force_z(
  ...
  body_weight = _get_total_mass(env, asset_cfg) * 9.81
  return force_z / torch.clamp(body_weight.unsqueeze(-1), min=eps)


def foot_force_momentum(
  env: ManagerBasedRlEnv,
  sensor_name: str,
  ...
  """Penalize high vertical force while a foot is moving downward.

  Contact force is normalized by total body weight so the term is less
  sensitive to robot mass and mass randomization.
  """
  asset: Entity = env.scene[asset_cfg.name]
  foot_vel_z = asset.data.site_lin_vel_w[:, asset_cfg.site_ids, 2]
  force_z_norm = _normalized_foot_force_z(
    env,
    sensor_name=sensor_name,
    asset_cfg=asset_cfg,
    eps=eps,
  )
  if foot_vel_z.shape != force_z_norm.shape:
    raise ValueError(
      "foot_force_momentum requires foot site velocities and contact forces "
      f"with matching shape, got {foot_vel_z.shape} and {force_z_norm.shape}."
    )

  downward_speed = torch.relu(-foot_vel_z)
  excess_force = torch.relu(force_z_norm - force_threshold)
  cost = torch.sum(downward_speed * excess_force, dim=1)
  return cost


class foot_force_rate:
  """Penalize positive changes in mass-normalized vertical foot force."""

  def __init__(self, cfg: RewardTermCfg, env: ManagerBasedRlEnv) -> None:
    self._last_force_z_norm: torch.Tensor | None = None
    self._has_last = torch.zeros(env.num_envs, device=env.device, dtype=torch.bool)

  def __call__(
    self,
    env: ManagerBasedRlEnv,
    sensor_name: str,
    force_rate_threshold: float = 0.0,
    asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
    eps: float = 1.0e-6,
  ) -> torch.Tensor:
    force_z_norm = _normalized_foot_force_z(
    ...
    if (
      self._last_force_z_norm is None
      or self._last_force_z_norm.shape != force_z_norm.shape
    ):
      self._last_force_z_norm = force_z_norm.detach().clone()
      self._has_last = torch.ones(env.num_envs, device=env.device, dtype=torch.bool)
      return torch.zeros(env.num_envs, device=env.device)

    force_increase = torch.relu(
      force_z_norm - self._last_force_z_norm - force_rate_threshold
    )
    force_increase = torch.where(
      self._has_last.unsqueeze(-1),
      force_increase,
      torch.zeros_like(force_increase),
    )
    cost = torch.sum(force_increase, dim=1)
    self._last_force_z_norm = force_z_norm.detach().clone()
    self._has_last[:] = True
    return cost
  ...
  def reset(self, env_ids: torch.Tensor | slice | None = None) -> None:
    if env_ids is None:
      env_ids = slice(None)
    self._has_last[env_ids] = False


class foot_lateral_distance:
  """Relaxed log-barrier cost for narrow foot-site-frame lateral spacing.

  ``frame`` selects the yaw frame for the lateral axis:
  ``"foot"`` uses the midpoint yaw of the two foot site frames, ``"root"``
  uses the root yaw, and ``"pelvis"`` uses the first configured body yaw
  falling back to root yaw when no body is configured.
  ``barrier_start_factor`` is measured against the initial foot distance like
  ``soft_factor``: the cost is zero while the current distance stays above
  ``initial_distance * barrier_start_factor``. ``barrier_delta`` is measured in
  meters: if ``distance - min_distance`` is less than or equal to this margin,
  the relaxed quadratic branch is used.
  """

  def __init__(self, cfg: RewardTermCfg, env: ManagerBasedRlEnv) -> None:
    self._initial_distance_b = torch.zeros(env.num_envs, device=env.device)
    self._has_reference = torch.zeros(env.num_envs, device=env.device, dtype=torch.bool)
    asset_cfg = cfg.params.get("asset_cfg")
    if cfg.weight != 0.0 and isinstance(asset_cfg, SceneEntityCfg):
      frame = str(cfg.params.get("frame", "foot"))
      self._cache_initial_distance(env, asset_cfg=asset_cfg, frame=frame)

  def __call__(
    self,
    env: ManagerBasedRlEnv,
    soft_factor: float = 0.5,
    barrier_delta: float = 0.02,
    barrier_start_factor: float | None = None,
    frame: str = "foot",
    asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
  ) -> torch.Tensor:
    distance_b = self._lateral_distance_b(env, asset_cfg, frame=frame)
    if distance_b is None:
      return torch.zeros(env.num_envs, device=env.device)
    if self._initial_distance_b.shape != distance_b.shape:
      self._initial_distance_b = torch.zeros_like(distance_b)
      self._has_reference = torch.zeros_like(distance_b, dtype=torch.bool)

    if torch.any(~self._has_reference):
      self._cache_initial_distance(env, asset_cfg=asset_cfg, frame=frame)

    soft_factor = float(soft_factor)
    barrier_delta = float(barrier_delta)
    self._validate_soft_factor(soft_factor)
    self._validate_barrier_delta(barrier_delta)
    min_distance = self._initial_distance_b * soft_factor
    margin = distance_b - min_distance
    if barrier_start_factor is None:
      return -_relaxed_log_barrier(margin, barrier_delta)

    barrier_start_factor = float(barrier_start_factor)
    self._validate_barrier_start_factor(barrier_start_factor, soft_factor)
    start_distance = self._initial_distance_b * barrier_start_factor
    safe_margin = (start_distance - min_distance).clamp_min(
      torch.finfo(distance_b.dtype).eps
    )
    normalized_margin = margin / safe_margin
    normalized_delta = barrier_delta / safe_margin
    return (-_relaxed_log_barrier(normalized_margin, normalized_delta)).clamp(min=0.0)

  def _cache_initial_distance(
    self,
    env: ManagerBasedRlEnv,
    asset_cfg: SceneEntityCfg,
    frame: str,
    ...
    if self._initial_distance_b.shape != distance_b.shape:
      ...
      self._has_reference = torch.zeros_like(distance_b, dtype=torch.bool)
    missing_reference = ~self._has_reference
    self._initial_distance_b = torch.where(
      missing_reference,
      distance_b.detach(),
      self._initial_distance_b,
    )
    self._has_reference = self._has_reference | missing_reference

  @staticmethod
  def _validate_soft_factor(soft_factor: float) -> None:
    if not 0.0 < soft_factor <= 1.0:
      raise ValueError(
        f"foot_lateral_distance soft_factor must be in (0, 1], got {soft_factor!r}."
      )

  @staticmethod
  def _validate_barrier_delta(barrier_delta: float) -> None:
    if barrier_delta <= 0.0:
      raise ValueError(
        f"foot_lateral_distance barrier_delta must be positive, got {barrier_delta!r}."
      )

  @staticmethod
  def _validate_barrier_start_factor(
    barrier_start_factor: float, soft_factor: float
  ) -> None:
    if not soft_factor < barrier_start_factor <= 1.0:
      raise ValueError(
        "foot_lateral_distance barrier_start_factor must be in "
        f"(soft_factor, 1], got {barrier_start_factor!r} with "
        f"soft_factor={soft_factor!r}."
      )

  def _lateral_distance_b(
    self,
    env: ManagerBasedRlEnv,
    asset_cfg: SceneEntityCfg,
    *,
    ...
    foot_pos_w = asset.data.site_pos_w[:, site_ids, :]
    if foot_pos_w.shape[1] != 2:
      return None

    foot_delta_w = foot_pos_w[:, 0, :] - foot_pos_w[:, 1, :]
    if frame == "foot":
      foot_quat_w = asset.data.site_quat_w[:, site_ids, :]
      foot_yaw = self._yaw_from_quat(foot_quat_w)
      reference_yaw = torch.atan2(
        torch.sin(foot_yaw[:, 0]) + torch.sin(foot_yaw[:, 1]),
        torch.cos(foot_yaw[:, 0]) + torch.cos(foot_yaw[:, 1]),
      )
    elif frame == "pelvis":
      reference_yaw = self._pelvis_yaw(asset, asset_cfg)
    else:
      reference_yaw = self._yaw_from_quat(yaw_quat(asset.data.root_link_quat_w))
    return self._lateral_distance_from_yaw(foot_delta_w, reference_yaw)

  @staticmethod
  def _validate_frame(frame: str) -> None:
    if frame not in ("foot", "root", "pelvis"):
      raise ValueError(
        f"Unsupported foot_lateral_distance frame {frame!r}; "
        "expected 'foot', 'root', or 'pelvis'."
      )

  @classmethod
  def _pelvis_yaw(cls, asset: Entity, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    body_ids = asset_cfg.body_ids
    if isinstance(body_ids, list) and body_ids:
      return cls._yaw_from_quat(asset.data.body_link_quat_w[:, body_ids[0], :])
    return cls._yaw_from_quat(yaw_quat(asset.data.root_link_quat_w))

  @staticmethod
  def _lateral_distance_from_yaw(
    foot_delta_w: torch.Tensor,
    reference_yaw: torch.Tensor,
  ) -> torch.Tensor:
    ref_sin = torch.sin(reference_yaw)
    ref_cos = torch.cos(reference_yaw)
    foot_delta_mid_y = -ref_sin * foot_delta_w[:, 0] + ref_cos * foot_delta_w[:, 1]
    return torch.abs(foot_delta_mid_y)
...
class foot_pitch_pattern:
  ...
  def __init__(self, cfg: RewardTermCfg, env: ManagerBasedRlEnv):
    ...
    self._target_error_penalty_weight: float = float(
      p.get("target_error_penalty_weight", 0.0)
    )
  ...
  def __call__(
    ...
    target_error_penalty_weight: float = 0.0,
    ...
    if use_temporal_window:
      ...
      if self._mode == "heel":
        ...
        raw_err_sq_buf = err.square()
        norm_err_sq_buf = raw_err_sq_buf * self._inv_std_sq
        score_buf = torch.exp(-norm_err_sq_buf)
        term_buf = _score_with_target_error_penalty(
          norm_err_sq_buf,
          raw_err_sq=raw_err_sq_buf,
          penalty_weight=self._target_error_penalty_weight,
        )
        assert hs_scale is not None
        raw_score = (score_buf * temporal_w).sum(dim=-1)
        score = (term_buf * temporal_w).sum(dim=-1) * just_landed
        ...
        raw_score = (score_buf * temporal_w).sum(dim=-1)
        score = (term_buf * temporal_w).sum(dim=-1) * just_lifted
        reward = score.sum(dim=1)
    else:
      if self._mode == "heel":
        # Heel-strike should favor heel-down (forward cmd -> gx < 0).
        err = -direction.unsqueeze(-1) * gx - self._target_gx
        raw_err_sq = err.square()
        norm_err_sq = raw_err_sq * self._inv_std_sq
        raw_score = torch.exp(-norm_err_sq)
        term_score = _score_with_target_error_penalty(
          norm_err_sq,
          raw_err_sq=raw_err_sq,
          penalty_weight=self._target_error_penalty_weight,
        )
        score = term_score * just_landed
        assert hs_scale is not None
        reward = (hs_scale.unsqueeze(-1) * score).sum(dim=1)
      else:
        # Toe-off should favor toe-down (forward cmd -> gx > 0).
...
class joint_gait_pattern:
  ...
  def __call__(
    ...
    if use_temporal_window:
      ...
      raw_err_sq_buf = torch.square(signed_joint_buf - self._target_angle_rad)
      assert self._core._temporal_w_by_buf_idx is not None
      temporal_w = self._core._temporal_w_by_buf_idx[buf_idx].view(1, 1, -1)
      if self._running_threshold is not None and cmd_speed is not None:
        ...
        norm_err_sq_buf = raw_err_sq_buf * inv_std_sq
      else:
        norm_err_sq_buf = raw_err_sq_buf * self._inv_std_sq
      score_buf = torch.exp(-norm_err_sq_buf)
      term_buf = _score_with_target_error_penalty(
      ...
      term_score = (term_buf * temporal_w).sum(dim=-1)
    else:
      signed_joint_pos = joint_pos * self._axis_signs.unsqueeze(0)
      raw_err_sq = torch.square(signed_joint_pos - self._target_angle_rad)
      if self._running_threshold is not None and cmd_speed is not None:
        inv_std_sq = torch.where(
          cmd_speed >= self._running_threshold,
          self._inv_std_running_sq,
        ...
        norm_err_sq = raw_err_sq * inv_std_sq
      else:
        norm_err_sq = raw_err_sq * self._inv_std_sq
      score = torch.exp(-norm_err_sq)
      term_score = _score_with_target_error_penalty(
    ...
    reward = torch.sum(term_score * event_mask, dim=1)
...
def _compute_link_swing_pattern_scores(
  env: ManagerBasedRlEnv,
  *,
  std: float,
  sensor_name: str,
  ...
  command_snapshot = _get_biped_command_base_snapshot(
    env,
    command_name=command_name,
    command_threshold=max(float(lin_vel_y_threshold), float(yaw_threshold)),
  )
  small_vy = torch.abs(command_snapshot.vy) <= float(lin_vel_y_threshold)
  small_yaw = torch.abs(command_snapshot.yaw) <= float(yaw_threshold)
  cmd_mask = (small_vy & small_yaw).float().unsqueeze(-1)
  ...
  if num_legs == 0:
    ...
    if len(axis_signs) != num_legs:
      raise ValueError(
        "link_swing_pattern axis_signs list length must match selected bodies: "
    ...
    raise TypeError(
      "link_swing_pattern axis_signs must be float/int, list/tuple, or dict."
    )
...
class link_swing_pattern:
  """Reward a target link-frame gravity direction during sagittal swing."""
  ...
  def __init__(self, cfg: RewardTermCfg, env: ManagerBasedRlEnv):
    ...
    self._yaw_threshold: float = p.get("yaw_threshold", 0.05)
  ...
  def __call__(
    ...
    reward_per_leg, mask, active = _compute_link_swing_pattern_scores(
      env,
      std=self._std,
      sensor_name=self._sensor_name,
      command_name=self._command_name,
...
class joint_effort_limits:
  ...
  def __init__(self, cfg: RewardTermCfg, env: ManagerBasedRlEnv) -> None:
    ...
    asset: Entity = env.scene[self._asset_name]
    self._torque_selection = resolve_torque_channels(asset, asset_cfg, env.device)

    # Cache mapping from selected local ctrl ids to columns for explicit overrides.
    selected_id_to_col = self._torque_selection.selected_id_to_col
  ...
  def __call__(
    ...
    asset: Entity = env.scene[self._asset_name]
    actuator_force, effort_limit = read_torque_and_limits(
      env, asset, self._torque_selection
    )
    actuator_force = torch.abs(actuator_force)
    ...
    # For custom actuators that expose motor-space torque, replace joint-space
    # torque so that force and limit are in the same space.
    for actuator, src_cols, dst_cols in self._explicit_overrides:
      if self._has_motor_space:
        ...
        if isinstance(motor_torque, torch.Tensor):
...
class joint_torque_rate_l2:
  ...
  contribute on a comparable scale. By default ``tau`` is the actuator output
  reported by the simulator. Set ``reference_torque=True`` to instead penalize
  the controller torque reference sent to the actuator/driver: explicit torque
  actuators are read from ``ctrl`` while implicit position actuators are
  recomputed from their delayed position reference and current state.
  """
  ...
  def __init__(self, cfg: RewardTermCfg, env: ManagerBasedRlEnv) -> None:
    ...
    self.substep_enabled = bool(cfg.params.get("per_substep", False))
    self._use_reference_torque = bool(cfg.params.get("reference_torque", False))
    ...
    selected_id_to_col = self._torque_selection.selected_id_to_col
    self._explicit_overrides: list[tuple[object, torch.Tensor, torch.Tensor]] = []
    self._motor_torque_overrides: list[tuple[object, torch.Tensor, torch.Tensor]] = []
    reference_ctrl_global_ids: list[torch.Tensor] = []
    reference_ctrl_dst_cols: list[torch.Tensor] = []
    reference_joint_target_ids: list[torch.Tensor] = []
    reference_joint_global_ctrl_ids: list[torch.Tensor] = []
    reference_joint_dst_cols: list[torch.Tensor] = []
    reference_joint_stiffness: list[torch.Tensor] = []
    reference_joint_damping: list[torch.Tensor] = []
    reference_joint_effort_limit: list[torch.Tensor] = []
    reference_tendon_target_ids: list[torch.Tensor] = []
    reference_tendon_global_ctrl_ids: list[torch.Tensor] = []
    reference_tendon_dst_cols: list[torch.Tensor] = []
    reference_tendon_stiffness: list[torch.Tensor] = []
    reference_tendon_damping: list[torch.Tensor] = []
    reference_tendon_effort_limit: list[torch.Tensor] = []
    has_motor_space = False
    for actuator in asset.actuators:
      force_limit = getattr(actuator, "force_limit", None)
      if hasattr(actuator, "last_motor_torque"):
      ...
      for j, local_id in enumerate(actuator.ctrl_ids.tolist()):
        ...
        if col is not None:
          ...
          dst_cols.append(col)
      if src_cols:
        src_tensor = torch.tensor(src_cols, device=env.device, dtype=torch.long)
        dst_tensor = torch.tensor(dst_cols, device=env.device, dtype=torch.long)
        if force_limit is not None or hasattr(actuator, "last_motor_torque"):
          self._explicit_overrides.append((actuator, src_tensor, dst_tensor))
        if hasattr(actuator, "last_motor_torque"):
          self._motor_torque_overrides.append((actuator, src_tensor, dst_tensor))
        if self._use_reference_torque and not hasattr(actuator, "last_motor_torque"):
          global_ctrl_ids = asset.data.indexing.ctrl_ids[actuator.ctrl_ids[src_tensor]]
          if isinstance(actuator, BuiltinPositionActuator):
            effort_limit = actuator.cfg.effort_limit
            effort_limit = float("inf") if effort_limit is None else effort_limit
            if actuator.transmission_type == TransmissionType.JOINT:
              reference_joint_target_ids.append(actuator.target_ids[src_tensor])
              reference_joint_global_ctrl_ids.append(global_ctrl_ids)
              reference_joint_dst_cols.append(dst_tensor)
              reference_joint_stiffness.append(
                torch.full_like(src_tensor, actuator.cfg.stiffness, dtype=torch.float)
              )
              reference_joint_damping.append(
                torch.full_like(src_tensor, actuator.cfg.damping, dtype=torch.float)
              )
              reference_joint_effort_limit.append(
                torch.full_like(src_tensor, effort_limit, dtype=torch.float)
              )
            elif actuator.transmission_type == TransmissionType.TENDON:
              reference_tendon_target_ids.append(actuator.target_ids[src_tensor])
              reference_tendon_global_ctrl_ids.append(global_ctrl_ids)
              reference_tendon_dst_cols.append(dst_tensor)
              reference_tendon_stiffness.append(
                torch.full_like(src_tensor, actuator.cfg.stiffness, dtype=torch.float)
              )
              reference_tendon_damping.append(
                torch.full_like(src_tensor, actuator.cfg.damping, dtype=torch.float)
              )
              reference_tendon_effort_limit.append(
                torch.full_like(src_tensor, effort_limit, dtype=torch.float)
              )
          elif isinstance(actuator, (BuiltinMotorActuator, IdealPdActuator)):
            reference_ctrl_global_ids.append(global_ctrl_ids)
            reference_ctrl_dst_cols.append(dst_tensor)
    self._has_motor_space = has_motor_space
    self._reference_ctrl_global_ids = self._cat_or_none(reference_ctrl_global_ids)
    self._reference_ctrl_dst_cols = self._cat_or_none(reference_ctrl_dst_cols)
    self._reference_joint_target_ids = self._cat_or_none(reference_joint_target_ids)
    self._reference_joint_global_ctrl_ids = self._cat_or_none(
      reference_joint_global_ctrl_ids
    )
    self._reference_joint_dst_cols = self._cat_or_none(reference_joint_dst_cols)
    self._reference_joint_stiffness = self._cat_or_none(reference_joint_stiffness)
    self._reference_joint_damping = self._cat_or_none(reference_joint_damping)
    self._reference_joint_effort_limit = self._cat_or_none(reference_joint_effort_limit)
    self._reference_tendon_target_ids = self._cat_or_none(reference_tendon_target_ids)
    self._reference_tendon_global_ctrl_ids = self._cat_or_none(
      reference_tendon_global_ctrl_ids
    )
    self._reference_tendon_dst_cols = self._cat_or_none(reference_tendon_dst_cols)
    self._reference_tendon_stiffness = self._cat_or_none(reference_tendon_stiffness)
    self._reference_tendon_damping = self._cat_or_none(reference_tendon_damping)
    self._reference_tendon_effort_limit = self._cat_or_none(
      reference_tendon_effort_limit
    )

    nu = self._torque_selection.num_channels
    ...
    if not isinstance(decimation, int):
      decimation = 1
    self._substep_torque = torch.zeros(
      (max(1, decimation), env.num_envs, nu),
      device=env.device,
      dtype=torch.float,
    )
  ...
  @staticmethod
  def _cat_or_none(tensors: list[torch.Tensor]) -> torch.Tensor | None:
    return torch.cat(tensors) if tensors else None

  def _read_torque(self, asset: Entity) -> torch.Tensor:
    if self._use_reference_torque:
      return self._read_reference_torque(asset)
    return self._read_sim_torque(asset)

  def _read_sim_torque(self, asset: Entity) -> torch.Tensor:
    torque, _ = read_torque_and_limits(self._env, asset, self._torque_selection)
    for actuator, src_cols, dst_cols in self._motor_torque_overrides:
      motor_torque = getattr(actuator, "last_motor_torque", None)
      if isinstance(motor_torque, torch.Tensor):
        torque[:, dst_cols] = motor_torque[:, src_cols]
    return torque

  def _read_reference_torque(self, asset: Entity) -> torch.Tensor:
    torque = self._read_sim_torque(asset)
    ctrl = asset.data.data.ctrl
    if self._reference_ctrl_global_ids is not None:
      torque[:, self._reference_ctrl_dst_cols] = ctrl[
        :, self._reference_ctrl_global_ids
      ]
    if self._reference_joint_global_ctrl_ids is not None:
      assert self._reference_joint_target_ids is not None
      assert self._reference_joint_dst_cols is not None
      assert self._reference_joint_stiffness is not None
      assert self._reference_joint_damping is not None
      assert self._reference_joint_effort_limit is not None
      target = ctrl[:, self._reference_joint_global_ctrl_ids]
      pos = asset.data.joint_pos[:, self._reference_joint_target_ids]
      vel = asset.data.joint_vel[:, self._reference_joint_target_ids]
      ref = self._reference_joint_stiffness * (target - pos)
      ref -= self._reference_joint_damping * vel
      ref = torch.clamp(
        ref,
        -self._reference_joint_effort_limit,
        self._reference_joint_effort_limit,
      )
      torque[:, self._reference_joint_dst_cols] = ref
    if self._reference_tendon_global_ctrl_ids is not None:
      assert self._reference_tendon_target_ids is not None
      assert self._reference_tendon_dst_cols is not None
      assert self._reference_tendon_stiffness is not None
      assert self._reference_tendon_damping is not None
      assert self._reference_tendon_effort_limit is not None
      target = ctrl[:, self._reference_tendon_global_ctrl_ids]
      pos = asset.data.tendon_len[:, self._reference_tendon_target_ids]
      vel = asset.data.tendon_vel[:, self._reference_tendon_target_ids]
      ref = self._reference_tendon_stiffness * (target - pos)
      ref -= self._reference_tendon_damping * vel
      ref = torch.clamp(
        ref,
        -self._reference_tendon_effort_limit,
        self._reference_tendon_effort_limit,
      )
      torque[:, self._reference_tendon_dst_cols] = ref
    return torque

  def _read_effort_limit(self, env: ManagerBasedRlEnv) -> torch.Tensor:
    asset: Entity = env.scene[self._asset_name]
    _, effort_limit = read_torque_and_limits(env, asset, self._torque_selection)
  ...
  def _grow_substep_torque_buffer(self) -> None:
    extra = torch.zeros_like(self._substep_torque)
    self._substep_torque = torch.cat((self._substep_torque, extra), dim=0)
  ...
  def record_substep(
    ...
    soft_ratio: float | None = None,
    eps: float | None = None,
    per_substep: bool | None = None,
    reference_torque: bool | None = None,
    asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
  ) -> None:
    del (
      soft_ratio,
      ...
      reference_torque,
      asset_cfg,
    )  # Only needed when reducing buffered torques.
    if self._substep_count >= self._substep_torque.shape[0]:
      self._grow_substep_torque_buffer()
    asset: Entity = env.scene[self._asset_name]
    self._substep_torque[self._substep_count].copy_(self._read_torque(asset))
    self._substep_count += 1

  def compute_substep_reward(
    self,
    env: ManagerBasedRlEnv,
    soft_ratio: float | None = None,
    eps: float | None = None,
    ...
    del per_substep, reference_torque, asset_cfg  # Mapping is cached at initialization.
    if self._substep_count == 0:
      return self(env, soft_ratio=soft_ratio, eps=eps)
    ...
    effort_limit = self._read_effort_limit(env)
    denom = effort_limit * ratio + denom_eps
    torques = self._substep_torque[: self._substep_count]
    torque_sequence = torch.cat((self._prev_torque.unsqueeze(0), torques), dim=0)
    delta = torque_sequence[1:] - torque_sequence[:-1]
    rate = torch.sum(torch.square(delta / denom.unsqueeze(0)), dim=2).sum(dim=0)
    self._prev_torque.copy_(torques[-1])
    self._substep_count = 0
    return rate
  ...
  def __call__(
    ...
    asset: Entity = env.scene[self._asset_name]
    torque = self._read_torque(asset)
    effort_limit = self._read_effort_limit(env)
  ...
  def reset(self, env_ids: torch.Tensor | slice) -> None:
    ...
    self._substep_torque[:, env_ids] = 0.0


class fourbar_constraint_barrier:
  """Compatibility placeholder for non-V4 tasks in the shared no-fourbar package."""

  def __init__(self, *_: object, **__: object) -> None:
    raise RuntimeError(
      "fourbar_constraint_barrier is not available in this shared V4 package. "
      "The copied V4 config does not use this reward."
    )

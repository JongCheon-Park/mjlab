...
@dataclass(kw_only=True)
class JointPositionHoldActionCfg(ActionTermCfg):
  """Configuration for holding joints at their default positions."""

  actuator_names: tuple[str, ...] | list[str]
  """Actuator names to hold at the default joint position."""

  def build(self, env: ManagerBasedRlEnv) -> JointPositionHoldAction:
    return JointPositionHoldAction(self, env)


class JointPositionHoldAction(ActionTerm):
  """Hold joints at their configured default positions."""

  cfg: JointPositionHoldActionCfg
  _entity: Entity

  def __init__(self, cfg: JointPositionHoldActionCfg, env: ManagerBasedRlEnv):
    super().__init__(cfg=cfg, env=env)
    ...
    if len(target_names) == 0:
      raise ValueError(
        f"No joints found for hold action with expressions: {cfg.actuator_names}"
      )
    self._target_ids = torch.tensor(target_ids, device=self.device, dtype=torch.long)
    self._target_names = target_names
  ...
  def apply_actions(self) -> None:
    encoder_bias = self._entity.data.encoder_bias[:, self._target_ids]
    target = self._entity.data.default_joint_pos[:, self._target_ids] - encoder_bias
    self._entity.set_joint_position_target(target, joint_ids=self._target_ids)
